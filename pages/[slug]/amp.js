...
export default function SinglePost({ item }) {
  ...
  return (
    ...
    <header className="sticky top-0 bg-white/75 md:bg-white backdrop-blur-3xl">
      <div className="flex items-center justify-between container mx-auto px-5 py-4">
        ...
        <button className="md:hidden" on='tap:sidebar1.toggle'>
            ...
            {categories.nodes.map((category) => {
              ...
              return (
                <div className="py-4" key={slug}>
                  <a href={`/category/${slug}`} className="px-3 py-1 bg-black text-white text-sm font-bold rounded-full">
                    {name}
                  </a>
                </div>
              );
